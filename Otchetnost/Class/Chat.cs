...
namespace Otchetnost
{
    ...
    public class Chat
    {
        ...
        string sqlInsertGroupMessage = "INSERT INTO chat_global(`user_id`,`text`) VALUES(@sql_user_id, @sql_text);";
        string sqlInsertDirectMessage = "INSERT INTO chat_global(`user_id`,`text`) VALUES(@sql_user_id, @sql_text);";
        ...
        string sqlSelectAllGroupMessage = "SELECT ch.id, ch.user_id, u.login, ch.`text`, ch.`date` FROM chat_global AS ch JOIN users AS u ON u.id = ch.user_id WHERE ch.id > @sql_id;";
        ...
        {
            //SQL.Insert<dynamic>(sqlInsertGroupMessage, new { sql_user_id = user_id, sql_text = text }, SQL.CONNECTION_STRING);
            //var ms = SQL.Select<Message, dynamic>(sqlSelectGroupMessage, new { sql_user_id = user_id }, SQL.CONNECTION_STRING)[0];

            return null;
        }

        public Message SendDirect(string text, int user_id, int user_id_other)
        {
            //SQL.Insert<dynamic>(sqlInsertDirectMessage, new { sql_user_id = user_id, sql_text = text }, SQL.CONNECTION_STRING);
        ...
        public List<Message> ReceiveAllGroup(int id)
        {
            //var ms = SQL.Select<Message, dynamic>(sqlSelectAllGroupMessage, new { sql_id = id }, SQL.CONNECTION_STRING);
